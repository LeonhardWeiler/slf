...
export function GameScreen() {
	...
	useEffect(() => {
		...
		if (entries.length > 0) {
			ws.send({
				...
				payload: {
					roundId,
					answers: entries.map(([categoryId, value]) => ({
						categoryId,
						value,
					})),
				},
	...
	const canBuzz =
		!isSpectator && lobby?.state === "Playing" && validation.valid;
				...
				<div className="flex h-48 w-48 items-center justify-center">
					...
					<div
						...
						className={`text-8xl font-bold tabular-nums min-h-[1em] ${
							isGo
								? "animate-countdown-go text-primary"
					...
					<>
						...
						{(buzzRejected === "incompleteAnswers" ||
							...
							<p className="text-sm text-destructive text-center">
								Du musst alle Kategorien gültig ausfüllen, bevor du buzzern
								kannst.
							</p>
						)}
