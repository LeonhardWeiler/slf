...
export function ReviewScreen() {
	...
	const mergeableCount = review.answers.filter(
		(a) => a.mergedInto === "",
	).length;
	...
	return (
		<div className="min-h-svh bg-background screen-pad">
			<div className="mx-auto w-full max-w-5xl space-y-4">
				...
				{buzzerName && (
					<p className="flex items-center justify-center gap-1.5 text-sm font-medium text-amber-800 dark:text-amber-300">
						<Bell className="h-4 w-4 shrink-0" />
						<span>
							<b>{buzzerName}</b> hat gestoppt
						</span>
					</p>
				)}
					...
					<CardContent className="pt-6 space-y-2">
						...
						{isHost && mergeMode && (
							<p className="text-xs text-muted-foreground px-1">
								Klicke eine Antwort, um sie mit „
								{playerName(
